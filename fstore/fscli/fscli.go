...
func (util *Util) GetOpt() error {
    ...
    help := func() {
        ...
        fmt.Printf("\n")
        fmt.Printf("Command list: hello, saveFile, loadFile, listFiles, deleteFile \n")
...
func (util *Util) GetHelloCmd(auth *dsrpc.Auth) (*fsapi.GetHelloResult, error) {
    var err error
    params := fsapi.NewGetHelloParams()
    params.Message = util.Message
    result := fsapi.NewGetHelloResult()
    err = dsrpc.Exec(util.URI, fsapi.GetHelloMethod, params, result, auth)
    if err != nil {
        return result, err
    }
    return result, err
