...
func (server *Server) RunService() error {
    ...
    contr := fscont.NewContr()
    dslog.LogDebug("data dir is", server.Params.DataDir)
    store := fsrec.NewStore(server.Params.DataDir)
    ...
    serv.Handler(fsapi.SaveMethod, contr.SaveHandler)
    serv.Handler(fsapi.LoadMethod, contr.LoadHandler)
    serv.Handler(fsapi.DeleteMethod, contr.DeleteHandler)
    serv.Handler(fsapi.ListMethod, contr.ListHandler)
